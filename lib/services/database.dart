...
class DatabaseService {
  ...
  Future addMessagesToDM(
      String msg, String? currentID, String? friendID) async {
  ...
  Future addCounsellor(String counsellorName) async {
    return await _userInfo
        .doc(user?.uid)
        .update({"counsellor": counsellorName});
  }

  Future updateSuccess() async {
    return await _userInfo.doc(user?.uid).update({"successful": true});
  }

  Future updateUser() async {
    return await _userInfo.doc(user?.uid).update({"acceptedPriv": true});
  }

  Future addProfileImageUrl(String url) async {
    return await _userInfo.doc(user?.uid).update({"profile_url": url});
  }
  ...
  Future createCalendarEvents(int? day, int? year, int? month, String? title,
    ...
    return await _userInfo.doc(user?.uid).collection('Calendar').add({
  ...
  Future addClubInformation(String clubName, String clubDesc, String advAddr,
      bool private, String password) async {
    List<String> stringList = [];
  ...
  Future addGroupMsgToDB(String msg, String? clubName) async {
    var now = DateTime.now();
  ...
  Future<void> updateClubMemberList(String clubName) async {
    return _userClubInfo.doc(clubName).update({
      'club_members': FieldValue.arrayUnion([user?.uid])
    });
  }

  Future<void> updateClubAdminList(String clubName) async {
    return _userClubInfo.doc(clubName).update({
      'admin_members': FieldValue.arrayUnion([user?.uid])
    });
  }

  Future<void> updateClubAdminList2(String? clubName, String uid) async {
    return _userClubInfo.doc(clubName).update({
      'admin_members': FieldValue.arrayUnion([uid])
    });
  }

  Future<void> removeClubMemberList(String? clubName) async {
    return _userClubInfo.doc(clubName).update({
      'club_members': FieldValue.arrayRemove([user?.uid])
    });
  }

  Future<void> removeClubMemberList2(String? clubName, String uid) async {
    return _userClubInfo.doc(clubName).update({
      'club_members': FieldValue.arrayRemove([uid])
    });
  }

  Future addClubProfileImageUrl(String? url, String clubName) async {
    return await _userClubInfo.doc(clubName).update({"profile_url": url});
  }

  Future updatePrivateOption(bool private, String clubName) async {
    return await _userClubInfo.doc(clubName).update({"private": private});
  }

  Future updatePasswordForPrivate(String password, String clubName) async {
    return await _userClubInfo
        .doc(clubName)
        .update({"private_password": password});
  }

  Future updateClubDesc(String desc, String clubName) async {
    return await _userClubInfo.doc(clubName).update({"club_desc": desc});
  }
  ...
  Future addCounsellorInfo(
      String counName, String counStds, String counEmail) async {
  ...
  Future<void> updateCounsellorMemberList(String counsellorName) async {
    return _userCounsInfo.doc(counsellorName).update({
      'club_members': FieldValue.arrayUnion([user?.uid])
    });
  }
